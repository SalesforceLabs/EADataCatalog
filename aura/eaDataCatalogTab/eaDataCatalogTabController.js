({
    ...
    handleEditAsset: function(component, event, helper) {
        ...
        let url = '/analytics/wave/wave.apexp';
        
        if (asset.type === 'recipe') {
           
            url += '#dataPrep/recipe=' + asset.id;
            
        } else {
            
            url += '?path=' + asset.type + '/' + asset.id;
        }
    ...
    handleAssetSearchTermChange: function(component, event, helper) {
        ...
        for (var key in itemMap) {
	        ...
	        if (term && term.length > 2) {
                if (item.label && item.label.toLowerCase().indexOf(term) >= 0) {
                    matches.push(item);
                    item.highlighted = true;
                    item.parent.expanded = true;
                } else if (item.name && item.name.toLowerCase().indexOf(term) >= 0) {
                    matches.push(item);
                    item.highlighted = true;
                    item.parent.expanded = true;                    
                } else if (item.id && item.id.indexOf(term) >= 0) {
                    matches.push(item);
                    item.highlighted = true;
                    item.parent.expanded = true;
                }
    ...
    handleArrangeNetwork: function(component, event, helper) {
        helper.handleArrangeNetwork(component);
    },
